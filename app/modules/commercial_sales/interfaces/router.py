...
from app.modules.commercial_sales.application.services import commercial_service
...
async def qualify_lead_endpoint(lead_id: int, payload: LeadQualify, db: AsyncSession = Depends(get_db)):
    lead = await commercial_service.qualify_lead(db, lead_id, payload.icp_score)
    if not lead:
        raise HTTPException(status_code=404, detail="Lead not found")
    return lead
...
async def win_lead_endpoint(
    ...
    lead = await commercial_service.win_lead(db, lead_id, proposal_id)
    if not lead:
...
async def accept_proposal_endpoint(proposal_id: int, db: AsyncSession = Depends(get_db)):
    proposal = await commercial_service.accept_proposal(db, proposal_id)
    if not proposal:
...
async def activate_subscription_endpoint(
    ...
    sub = await commercial_service.activate_subscription(db, contract_id, payload.tier, payload.seats)
    if not sub:
        raise HTTPException(
            status_code=400,
...
async def churn_subscription_endpoint(
    ...
    sub = await commercial_service.churn_subscription(db, subscription_id, payload.reason)
    if not sub:
        raise HTTPException(status_code=404, detail="Subscription not found")
    await event_bus.emit(Event(
...
async def start_onboarding_endpoint(
    ...
    onboarding = await commercial_service.start_onboarding(db, contract_id, payload.steps)
    if not onboarding:
        raise HTTPException(status_code=404, detail="Contract not found")
    return onboarding
...
async def complete_step(onboarding_id: int, step_index: int, db: AsyncSession = Depends(get_db)):
    onboarding = await commercial_service.complete_onboarding_step(db, onboarding_id, step_index)
    if not onboarding:
...
async def terminate_contract_endpoint(
    ...
    contract = await commercial_service.terminate_service_contract(db, contract_id, payload.reason)
    if not contract:
        raise HTTPException(
            status_code=400,
