...
from app.modules.strategic_planning.application.services import strategic_service
...
async def update_objective_progress(
    ...
    progress = strategic_service.calculate_objective_progress(obj)
...
async def create_management_review(
    ...
    report = await strategic_service.create_management_review_report(
        db=db,
        code=payload.code,
        title=payload.title,
...
async def get_current_qms_scope(db: AsyncSession = Depends(get_db)):
    scope = await strategic_service.get_active_qms_scope(db)
    if not scope:
...
async def create_qms_scope(payload: QmsScopeCreate, db: AsyncSession = Depends(get_db)):
    ...
    existing = await strategic_service.get_active_qms_scope(db)
    if existing:
