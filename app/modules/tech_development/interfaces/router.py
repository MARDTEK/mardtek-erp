...
from app.modules.tech_development.application.services import tech_service
from app.modules.tech_development.infrastructure.repositories import (
    deployment_record_repo,
    release_plan_repo,
    roadmap_repo,
    solution_sunset_repo,
    tech_spec_repo,
    risk_matrix_repo,
    qa_test_report_repo,
    uat_signoff_repo,
...
async def list_releases(
    ...
    if status:
        return await tech_service.get_release_by_status(db, status)
        ...
        await event_bus.emit(
            Event(
                ...
                payload={
                    ...
                    "pass_rate": tech_service.calculate_qa_pass_rate(report),
                },
...
async def create_deployment(payload: DeploymentRecordCreate, db: AsyncSession = Depends(get_db)):
    record = await tech_service.create_deployment_record(
        db=db,
        code=payload.code,
